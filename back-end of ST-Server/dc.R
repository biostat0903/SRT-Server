...
ref_path <- "/net/mulan/disk2/yasheng/stwebProject/02_data/00_ref/"
...
dc.check <- function(data_path = NULL,               ## String: output path for qc procedure
  ...
  if (!is.null(species) & !is.null(status)){
    ...
    ref_type <- 'sc'
    sc_dir <- paste0(ref_path, "/scRNA_ref/", species, "/", status)
  } else{
...
dc.call <- function(out_path                         ## String: output path for dc procedure
){
  ...
  ## format results
  dc_result_list <- plyr::llply(seq_along(CARD_obj_list), function(a){
    
    dc_result <- list()
    dc_result[["Proportion_CARD"]] <- CARD_obj_list[[a]]@Proportion_CARD
    dc_result[["location"]] <- CARD_obj_list[[a]]@spatial_location
    dc_result[["sample_info"]] <- 
      data.frame(sample = a,
                 cell = rownames(CARD_obj_list[[a]]@spatial_location))
    return(dc_result)
  })
  dc_refine_result_list <- plyr::llply(seq_along(CARD_obj_list), function(a){
    
    dc_refine_result <- list()
    dc_refine_result[["refined_expression"]] <- CARD_obj_list[[a]]@refined_expression
    dc_refine_result[["refined_prop"]] <- CARD_obj_list[[a]]@refined_prop
    
    return(dc_refine_result)
  })
  ...
  save(dc_result_list, file = resule_path)
  refine_resule_path <- paste0(out_path, '/dc_refine_result_list.RData')
  save(dc_refine_result_list, file = refine_resule_path)
  write.table(c(resule_path, refine_resule_path, ref_type, scMapping_path),
              file = paste0(out_path, "/dc_call_file.txt"), 
...
dc.post <- function(out_path                         ## String: output path for dc procedure
){
  
  ## load data
  call_file <- fread2(paste0(out_path, "/dc_call_file.txt"), header = F)[, 1]
  ...
  deconv <- lapply(dc_result_list, function(dc_result){
    sample_info <- dc_result$sample_info
    deconv <- cbind(sample_info, 
                    dc_result$Proportion_CARD[sample_info$cell,])
  }) %>% Reduce("rbind", .)

  result_dir <- paste0(out_path, "/dc_result/")
  ...
  if (!is.null(scMapping_path)) {
    ...
    scmap_path <- lapply(seq_along(sce_count_list), function(a){
      ...
      sce_result_dir <- paste0(out_path, "/dc_result/sample", a, "/")
  ...
  write.table(c(paste0(result_dir, "/deconv_",ref_type,".txt"), 
                scmap_path), 
              file = paste0(out_path, "/dc_post_file.txt"), 
              sep = "\t", row.names = F, col.names = F, quote = F)
  return(0)
}
...
output_path <- "/net/mulan/disk2/yasheng/stwebProject/test/dc"
dc.check(data_path = data_path,
         species = "Human",
         status = "Breast_BRCA",
         # marker_list = NULL,
         num_grid = 20000,
...
dc.call(out_path = output_path)
dc.post(out_path = output_path)
