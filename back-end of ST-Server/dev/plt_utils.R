  ...
  if (is.null(marker_gene_list)){
    ...
    marker_gene_sel <- plyr::llply(count_list, function(a){
      ...
      seurat_obj_s <- FindVariableFeatures(seurat_obj_s, 
                                           ...
                                           verbose = FALSE)
      hvg_s <- seurat_obj_s@assays$RNA@var.features
      return(hvg_s)
    })
  } else {
...
bubble.plot <- function(data_path1,                   ## String: output path of dr_cl procedure 
  ...
  ## Load cluster 
  dr_cl_post_file <- paste0(data_path1, "/", mode_usage, "_post_file.txt")
  dr_cl_clus_file <- read.table(dr_cl_post_file)[1, 1]
  dr_cl_clus <- fread2(dr_cl_clus_file)
  setting_num <- ncol(dr_cl_clus) - 2
  setting_str <- sub("cluster_", "", colnames(dr_cl_clus)[3: ncol(dr_cl_clus)])
  ...
  bb_datt <- plyr::alply(c(1: setting_num), 1, function(a){
    clus_s <- dr_cl_clus[, c(1, 2, 2+a)]
    bb_datt_s <- bubble.process(data_path = data_path, 
                                    clus = clus_s, 
                                    marker_gene = marker_gene_sel,
                                    do_scale = T)
  }) %>% Reduce("rbind", .)
...
# Function 6: Visualize location plot data 
loc.plot <- function(data_path1,                   ## String: output path of dr_cl procedure 
                     data_path2,                   ## String: output path of qc procedure
  ...
  ## Load cluster 
  dr_cl_post_file <- paste0(data_path1, "/", mode_usage, "_post_file.txt")
  dr_cl_clus_file <- read.table(dr_cl_post_file)[, 1]
  if(mode_usage == "dr_cl"){
  ...
  if(vis_type == "cell_type"){
    dr_cl_clus <- fread2(dr_cl_clus_file[1])
  } else {
    dr_cl_clus <- fread2(dr_cl_clus_file[2])
  }
  clus_name <- colnames(dr_cl_clus)[-c(1, 2)]
  setting_num <- ncol(dr_cl_clus) - 2
  setting_str <- sub("cluster_", "", colnames(dr_cl_clus)[3: ncol(dr_cl_clus)])
  
  ## process data
  datt <- loc.process(data_path2)
  sample_size <- length(unique(dr_cl_clus$sample))
  ## location plot
  ct_loc_list <- lapply(clus_name, function(cl){
    
    datt$cluster <- dr_cl_clus[, cl] %>% 
      factor(., levels = sort(unique(.)))
    if (vis_type == "cell_type"){
