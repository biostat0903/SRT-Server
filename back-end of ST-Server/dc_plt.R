...
# Function 1: Process DC data
base.process <- function(data_path){
  
  # 1. load data
  call_path <- paste0(data_path, "/dc_call_file.txt")
  ...
  base_df <- lapply(dc_result_list, function(dc_result){
    sample_info <- dc_result[["sample_info"]]
    sample_info$sample <- paste0("Sample", sample_info$sample)
    base_df_a <- cbind(sample_info,
                       dc_result[["location"]][sample_info$cell, , drop = F],
                       dc_result[["Proportion_CARD"]][sample_info$cell, , drop = F])
    colnames(base_df_a)[1:4] <- c("sample", "cell", "loc_x", "loc_y")
    
    return(base_df_a)
  }) %>% Reduce("rbind", .)
  
  print("Base DC data formatted!")
  return(base_df)
  
}

# Function 2: Process refine DC data
refine.process <- function(data_path,
  ...
  ## format base df
  base_df <- lapply(seq_along(dc_refine_result_list), function(a){
    
    dc_result_a <- dc_refine_result_list[[a]]
    refined_prop <- dc_result_a[["refined_prop"]]
  ...
  print("Base redined DC data formatted!")
...
pie.plot <- function(data_path,
  ...
  # plot
  dc_pie_plt <- pie.visualize(datt = pie_df,
                              pie_scale = pie_scale,
                              color_in = COLOR_USE,
  ...
  if(out_figure == TRUE){
    ...
    ggsave(filename = paste0(out_path, "/dc_result/dc_pie.tiff"),
           plot = dc_pie_plt,
           height = pie_ht, width = pie_wt, units = "in", 
           dpi = 300, compression = "lzw")
    system(paste0("gzip -f ", out_path, "/dc_result/dc_pie.tiff"))
  }
...
propPattern.process <- function(base_df,
  ...
  print(paste0("DC data formatted for ", paste(ct_sel, collapse = ", "), "!"))
  return(m_scale_df)
}
...
propPattern.plot <- function(data_path,
  ...
  # plot
  dc_propPattern <- propPattern.visualize(datt = m_scale_df,
                                          point_size = 0.5,
                                          add_theme = out_figure)
  if(out_figure == TRUE){
    ...
    ggsave(filename = paste0(out_path, "/dc_result/dc_propPattern.tiff"),
           plot = dc_propPattern,
           height = pp_ht, width = pp_wt, units = "in", dpi = 300, compression = "lzw")
    system(paste0("gzip -f ", out_path, "/dc_result/dc_propPattern.tiff"))
  }
  
  return(dc_propPattern)
}
...
refine.propPattern.plot <- function(data_path,
  ...
  if(out_figure == TRUE){
    ...
    pp_ht <- ceiling(n_ct / 5) * 5.5
    pp_wt <- min(5, n_ct) * 3 + 1
    ggsave(filename = paste0(out_path, "/dc_result/dc_refine_propPattern.tiff"),
           plot = dc_propPattern,
           height = pp_ht, width = pp_wt, units = "in", dpi = 300, compression = "lzw")
    system(paste0("gzip -f ", out_path, "/dc_result/dc_refine_propPattern.tiff"))
  }
...
refine.feature.plot <- function(data_path, 
  ...
  ## inputs
  dc_refine_list <- refine.process(data_path,
                                   expr_norm = TRUE)
  base_df <- dc_refine_list[["base_df"]]
  expr_refine <- dc_refine_list[["expr_refine"]]
  ...
  if (out_figure == TRUE){
    ...
    ggsave(filename = paste0(out_path, "/dc_result/dc_refine_featurePlot.tiff"), 
           plot = feature_plt,
           height = ft_ht, width = ft_wt, units = "in", dpi = 300, compression = "lzw")
    # system(paste0("gzip -f ", out_path, "/dc_result/dc_refine_featurePlot.tiff"))
...
dcMapping.loc.plot <- function(data_path, 
                               out_path, 
                               point_size,
                               out_figure = FALSE
){
  ...
  if (out_figure == TRUE){
    ...
    ggsave(filename =  paste0(out_path, "/dc_result/dc_mapping_locPlot.tiff"), 
           plot = ct_plt,
           height = ct_ht, width = ct_wt, units = "in", dpi = 300, compression = "lzw")
    system(paste0("gzip -f ", out_path, "/dc_result/dc_mapping_locPlot.tiff"))
...
# Function 11
dc_plt.plot <- function(data_path1,                     ## String: output path for dc procedure 
                        data_path2,                     ## String: output path for qc procedure 
                        out_path,                       ## String: output path for dc_plt procedure
  ...
  ## plot
  dc_pie_plt <- pie.plot(data_path = data_path1,
                         pie_scale = 0.3,
  ...
  prop_pattern_list <- propPattern.plot(data_path = data_path1,
                                        ...
                                        point_size = 0.2,
                                        out_figure = out_figures)
  dc_feature_plt <- feature.plot(data_path1 = data_path1,
                                 ...
                                 out_figure = out_figures)
  ## save data
  save(prop_pattern_list, dc_pie_plt, dc_feature_plt, 
       file = paste0(output_path, "/dc_result/plot.RData"))
  ...
  if (scMapping == TRUE){
    ...
    refine_prop_pattern_list <- refine.propPattern.plot(data_path = data_path1,
                                                    ...
                                                    out_figure = out_figures)
    save(prop_pattern_list, dc_pie_plt, dc_feature_plt, 
         refine_prop_pattern_list,file = paste0(output_path, "/dc_result/plot.RData"))
  } 
...
# ###################
# ### test code
# data_path1 <- "/net/mulan/disk2/yasheng/stwebProject/test/dc"
# data_path2 <- "/net/mulan/disk2/yasheng/stwebProject/test/qc"
# output_path <- "/net/mulan/disk2/yasheng/stwebProject/test/dc_plt"
# dc_plt.plot(data_path1 = data_path1,
#             data_path2 = data_path2,
#             out_path = output_path,
#             out_figures = TRUE)
