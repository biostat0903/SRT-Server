...
svgpattern.plot <- function(data_path1,                          ## String: output path for qc procedure
  ...
  if (out_figure == TRUE){
    ...
    svgpt_ht <- max(0.25*svg_num*sample_size, 2)
    svgpt_wt <- max(0.5*svg_num*sample_size, 4)
...
pattern.plot <- function(data_path, 
  ...
  pt_plt <- ggplot(tot_pattern_datt, aes(x = x, y = y, color = patternValue)) + 
    geom_point(alpha = 1, size = pointsize) + 
    scale_color_gradientn(colours = pal(5)) + 
    theme_bw() +
    theme(legend.position = "none",
  ...
  if (sample_size == 1) {
    ...
    pt_plt <- pt_plt + facet_wrap(~patternType, ncol = 4)
  } else {
    
    pt_plt <- pt_plt + facet_wrap(~sample + patternType, ncol = 4, 
                                  dir = "v")
  }
  ## output figure
  if (out_figure == TRUE){
    
    pt_ht <- max(0.25*pattern_num*sample_size, 2)
    pt_wt <- max(0.5*pattern_num*sample_size, 4)
...
qq.visualize <- function(datt
  ...
  plt <- ggplot(df1) + 
    ...
    geom_point(aes(expected, observed), 
               size = 0.2, col = "lightpink", alpha = 1)+
    ...
    facet_wrap(~ sample, nrow = sample_size, dir = "v", scales = "free")
...
qq.plot <- function(data_path, 
  ...
  if(out_figure== TRUE){
    ...
    qq_ht <- max(sample_size, 2)
    qq_wt <- max(0.5*sample_size, 2)
...
hist.visualize <- function (datt,
  ...
  if (sample_size == 1) {
    ...
    plt_list <- lapply(1: sample_size, function(a){
      ...
      plt_a <- plt_a + 
        annotate("text", x = 0.65*cutoff, y = 0.8, size = 5,
                 label = bquote(Number~of~Genes[P < .(cutoff)]:.(n_s)~.(p_s))) +
        annotate("text", x = 0.65*cutoff, y = 0.7, size = 5,
                 label = bquote(Number~of~Genes[P-adj < .(cutoff)]:.(n_as)~.(p_as)))
...
hist.plot <- function(cutoff = 0.01,
  ...
  if(out_figure == TRUE){
    ...
    hist_ht <- ifelse(sample_size == 1, 2, 2*floor(sqrt(sample_size)))
...
svg_plt.plot <- function(data_path1,                       ## String: output path for qc procedure
  ...
  save(svgpattern_plt, pattern_plt, qq_plt, hist_plt, 
       file = paste0(out_path, "/svg_result/plot.RData"))
...
svg_plt.plot(data_path1 = data_path,
             data_path2 = output_path,
             out_path = output_path,
             svg_num = 4,
             out_figures = TRUE)
