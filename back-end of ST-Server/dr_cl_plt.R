...
umap.process <- function(data_path 
  ...
  ## load umap file
  cl_post_file <- read.table(paste0(data_path, "/dr_cl_post_file.txt"))[,1]
  umap_outpath <- cl_post_file[2]
  load(umap_outpath)
  
  ## load labels
  cluster_label <- read.table(paste0(data_path, "/dr_cl_result/cluster_label.txt"), 
                              head = T) 
  ...
  ct_umap_list <- lapply(names(datt_list), function(a){
    ...
    lapply(cluster_name_a, function(cl){
      ...
      if(out_figure == TRUE){
        ...
        plt_ht <- 3
        plt_wt <- 4
        ggsave(filename = paste0(out_path, "/dr_cl_result/UMAP_ct_", cl, ".tiff"),
               plot = ct_umap_plt,
               height = plt_ht, width = plt_wt, units = "in", dpi = 300)
        system(paste0("gzip -f ", out_path, "/dr_cl_result/UMAP_ct_", 
                      cl, ".tiff"))
      }
      return(ct_umap_plt)
    })
...
# Function 4
dr_cl_plt.plot <- function(data_path1,                              ## String: output path of dr_cl procedure
                           data_path2,                              ## String: output path of qc procedure
                           out_path,                                ## String: output path of dr_cl_plt procedure
  ...
  save(umap_plt, loc_cl_plt, ft_plt, bb_plt,
       file = paste0(out_path, "/dr_cl_result/plot.RData"))
...
# dr_cl_plt.plot(data_path1 = output_path,
