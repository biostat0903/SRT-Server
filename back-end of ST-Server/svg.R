#! /usr/bin/env Rscript
# Identify spatially variable gene for st RNA-seq data
...
svg.check <- function(data_path = NULL,              ## String: output path of qc procedure
                      svg_method = NULL,             ## String: method for svg: {SPARK}, {SPARKX}, {SpatialDE}
  ...
  if(pattern_est == FALSE){
    ...
    write.table(c(svg_method, pattern_est), 
...
pattern.define <- function(exp_mat,
  ...
  pattern_dat <- setNames(cbind.data.frame(loc_info, rel_cent), 
                          c("x", "y", paste0("Pattern", c(1: clus_num))))
...
svg.call <- function(data_path = NULL,                ## String: output path of qc procedure
  ...
  pattern_est <- ifelse(check_file[2] == "FALSE", FALSE, TRUE)
  if (pattern_est == TRUE){
    
    pval_thr <- check_file[3] %>% as.numeric()
    clus_num <- check_file[4] %>% as.numeric()
  }
  ...
  ## define spg using different methods
  ### SPARK
  if (svg_method == "SPARK"){
    ...
    tot_svg_result <- plyr::llply(tot_list, function(a){
      exp_mat_s <- a[[1]]
      loc_info_s <- a[[2]]
      spark_s <- Spark.svg.test(exp_mat_s, as.data.frame(loc_info_s))
      colnames(spark_s) <- c("Gene", "Pval", "adjPval")
      return(spark_s)
    })
  }
  ### SpatialDE
  if (svg_method == "SpatialDE"){
    ...
    tot_svg_result <- plyr::llply(tot_list, function(a){
      exp_mat_s <- a[[1]]
      loc_info_s <- a[[2]]
      spatialDE_s <- SpatialDE.svg.test(exp_mat_s, loc_info_s)
      colnames(spatialDE_s) <- c("Gene", "Pval", "adjPval")
      return(spatialDE_s)
    })
  ...
  if (svg_method == "SPARKX"){
    ...
    tot_svg_result <- plyr::llply(tot_list, function(a){
      exp_mat_s <- a[[1]]
      loc_info_s <- a[[2]]
      sparkx_s <- SparkX.svg.test(t(exp_mat_s), loc_info_s)
      colnames(sparkx_s) <- c("Gene", "Pval", "adjPval")
      return(sparkx_s)
    })
  }
  ## output parameters
  ...
  if (pattern_est == TRUE){
    ...
    tot_pattern_result <- plyr::alply(c(1: sample_size), 1, function(a){
      ...
      pattern_dat_s <- pattern.define(exp_mat_s,
                                      loc_info_s,
                                      svg_result_s, 
                                      pval_thr,
                                      clus_num) %>%
    ...
    ## output data
    save(tot_svg_result, tot_pattern_result,
         file = svg_result_path)
  } else {
    
    ## output data
    save(tot_svg_result, file = svg_result_path)
  }
...
output_path <- "/net/mulan/disk2/yasheng/stwebProject/test/svg"
svg.check(data_path = data_path,
          svg_method = "SPARKX",
          pattern_est = TRUE,
          pval_thr = 0.01,
          clus_num = 3,
          out_path = output_path)
svg.call(data_path = data_path, out_path = output_path)
svg.post(out_path = output_path)
