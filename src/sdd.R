...
sdd.check <- function(data_path,                               ## String: output path of qc procedure
                      ...
                      BASS_dobatch = TRUE,                     ## Boolean: weather to do batch correct via harmony in BASS
  ...
  if (sdd_methods == "CL_jo-BASS"){
    ...
    write.table(c(sdd_methods, 
                  ...
                  BASS_initMethod, BASS_betaMethod, BASS_geneSelect, BASS_dobatch),
                file = paste0(out_path, "/sdd_check_file.txt"), 
...
SpatialPCA.call.func <- function(st_list,                      ## String: output path of qc procedure
  ...
  if (sample_size == 1) {
    ...
    spca_obj <- pca_obj
    ## sample information
    sample_info <- data.frame(sample = 1,
...
BASS.call.func <- function(st_list,                            ## String: output path of qc procedure
  ...
  dobatch <- ifelse(check_file[7] == "TRUE", T, F)
...
SpatialPCA.post.func <- function(data_path,                         ## String: output path of qc procedure
                                 out_path,                          ## String: output path of sdd procedure
                                 sdd_methods                        ## String: sdd method used
){
...
BASS.post.func <- function(data_path,                         ## String: output path of annot procedure
  ...
  cluster_label <- lapply(seq_along(cl_dom_list), function(a){
    ...
    clust_df$cluster_label <- factor(clust_df$cluster_label,
                                     labels = 1:length(unique(clust_df$cluster_label))) %>% 
      as.integer()
  ...
  domain_label <- lapply(seq_along(cl_dom_list), function(a){
    ...
    domain_df$domain <- factor(domain_df$domain,
                               labels = 1:length(unique(domain_df$domain))) %>% 
      as.integer()
  ...
  cluster_label_file <- paste0(result_dir, "/cluster_label.txt")
  domain_label_file <- paste0(result_dir, "/domain_label.txt")
...
sdd.post <- function(data_path = NULL,                        ## String: output path of annot procedure
  ...
  if (sdd_methods == "SDD_sPCA-SpatialPCA"){
    ...
    sdd_result <- SpatialPCA.post.func(data_path, out_path, sdd_methods)
  }
  if (sdd_methods == "CL_jo-BASS"){
    
    sdd_result <- BASS.post.func(data_path, out_path, sdd_methods)
  }
  return(0)
}
