...
Seurat.call.func <- function(st_list,                   ## String: output path of qc procedure
  ...
  if (sample_size == 1 | (sample_size != 1 & batch == "Merge")){
    ...
    #              overwrite = TRUE)
    save(seurat_obj, file = paste0(out_path, "/ct_pca_seurat_obj.RData"))
  } else {
    ...
    for (anchor in anchors){
      ...
      for (dim in dims){
        ...
        #              overwrite = TRUE)
        save(seurat_obj, file = paste0(out_path, "/ct_pca_seurat_obj_anchor", anchor, "_dim", dim, ".RData"))
      }
    }
...
scSorter.call.func <- function(st_list,                 ## String: output path of qc procedure
  ...
  clust_file <- paste0(result_dir, "/ct_annot_scsorter_cluster_label.txt")
...
Garnett.call.func <- function(train = FALSE,            ## Boolean: no training model
  ...
  clust_file <- paste0(result_dir, "/ct_annot_garnett_cluster_label.txt")
  sample_size <- length(st_list[["count_list"]])
  sample_num <- lapply(c(1:sample_size), function(s){
...
ct.call <- function(data_path,                         ## String: output path of qc procedure
  ...
  if (ct_methods == "CT_Annot-scSorter"){
    ...
    ct_result <- scSorter.call.func(st_list, out_path)
  }
  if (ct_methods == "CT_Annot-Garnett"){
    
    ct_result <- Garnett.call.func(TRUE, st_list, out_path)
  }
  return(0)
}
