...
dot.plt <- function(datt,
                    n_top = NULL,
                    title_in = NULL){
  #
  datt$GeneRatio <- lapply(1:nrow(datt), function(rr){
    eval(expr = parse(text = datt$GeneRatio[rr]))
  }) %>% unlist()
  datt$logP <- -log10(datt$p.adjust)
  datt_top <- dplyr::top_n(datt, n = n_top, wt = GeneRatio) 
  datt_top <- datt_top[order(datt_top$GeneRatio, 
                             datt_top$Count, 
                             datt_top$logP),]
  datt_top$Description <- factor(datt_top$Description, 
                                 levels = datt_top$Description)
  
  plt <- ggplot(datt_top, aes(x = Description, y = GeneRatio)) + 
    geom_point(aes(color = logP, size = Count)) + 
    xlab("") + ylab("GeneRatio") + ggtitle(title_in) + 
    scale_color_viridis_c() + 
    guides(color = guide_colorbar(title = "log10(P.adjust)")) + 
    coord_flip() + 
    theme_bw() + 
...
ora_plt.plot <- function(data_path,                ## String: output path of ora procedure
  ...
  if (pathway_db == "ALL"){
    ...
    dot_plt <- dot.plt(ora_res, 
                       n_top = 10,
                       title = pathway_db)
    if(out_figures == TRUE){
      
      ggsave(filename = paste0(result_dir, "/Dot_plot_", pathway_db, ".tiff"),
             plot = dot_plt,  
             width = 9, height = 5, units = "in", dpi = 300)
      if(zip_figures == TRUE){
        
        system(paste0("gzip -f ", result_dir, "/Dot_plot_",
                      pathway_db, ".tiff"))
      }
    }
  }
