...
base.process <- function(data_path1,
                         data_path2){
  
  #
  post_path <- paste0(data_path1, "/decon_post_file.txt")
  post_file <- read.table(post_path, header = F, sep = "\t")[, 1]
  result_path <- post_file[2]
  decon_result <- fread2(result_path)
  #
  source(paste0(method_path, "dev/io.R"))
  qc_param <- read.table(paste0(data_path2, "/qc_call_file.txt"),
                         header = F, sep = "\t")[, 1]
  ...
  spatial_data <- h5data.load(spatial_data_filename,
                              ...
                              coordinate = TRUE,
                              image = FALSE)
  coord_info <- lapply(1:sample_size, function(x){
    coordx <- as.data.frame(spatial_data[["coord_list"]][[x]])
    return(coordx)
  }) %>% Reduce("rbind", .)
  # format base df
  base_df <- cbind(decon_result[, 1:2],
                   coord_info[decon_result$cell,],
                   decon_result[, sort(colnames(decon_result)[-c(1:2)])])
  colnames(base_df)[1:4] <- c("sample", "cell", "loc_x", "loc_y")
...
refine.process <- function(data_path,
  ...
  refine_result_path <- call_file[2]
  load(refine_result_path) # CARD_refine_result_list
  ...
  base_df <- lapply(seq_along(decon_refine_result_list), function(a){
    ...
    refined_prop <- refined_prop[,sort(colnames(refined_prop))]
...
pie.visualize <- function(datt,
  ...
  datt$sample <- paste0("Sample ", datt$sample)
  plt <- ggplot() + 
...
pie.plot <- function(data_path1,
  ...
  # process plot data
  pie_df <- base.process(data_path1 = data_path1,
  ...
  if(out_figure == TRUE){
    ...
    ggsave(filename = paste0(out_path, "/decon_pie.tiff"),
           plot = decon_pie_plt,
           height = pie_ht, width = pie_wt, units = "in", 
           dpi = 300, compression = "lzw")
    if (zip_figure == TRUE){
      
      system(paste0("gzip -f ", out_path, "/decon_pie.tiff"))
    }
  }
...
propPattern.plot <- function(data_path1,
                             data_path2,
                             out_path,
                             ct_sel,
                             point_size,
  ...
  # process plot data
  base_df <- base.process(data_path1 = data_path1,
                          data_path2 = data_path2)
  ...
  if(out_figure == TRUE){
    ...
    pp_ht <- ceiling(n_ct / 5) * 3
    pp_wt <- min(5, n_ct) * 3 + 1
    ggsave(filename = paste0(out_path, "/decon_propPattern.tiff"),
           plot = decon_propPattern,
           height = pp_ht, width = pp_wt, units = "in",
           dpi = 300, compression = "lzw")
    if(zip_figure == TRUE){
      
      system(paste0("gzip -f ", out_path, "/decon_propPattern.tiff"))
    }
  }
...
refine.propPattern.plot <- function(data_path,
  ...
  if(out_figure == TRUE){
    ...
    pp_ht <- ceiling(n_ct / 5) * 3
    pp_wt <- min(5, n_ct) * 3 + 1
    ggsave(filename = paste0(out_path, "/decon_refine_propPattern.tiff"),
           plot = decon_propPattern,
           height = pp_ht, width = pp_wt, units = "in", 
           dpi = 300, compression = "lzw")
    if (zip_figure == TRUE){
      
      system(paste0("gzip -f ", out_path, "/decon_refine_propPattern.tiff"))
    }
  }
...
refine.feature.plot <- function(data_path, 
  ...
  if (out_figure == TRUE){
    ...
    ggsave(filename = paste0(out_path, "/decon_refine_featurePlot.tiff"), 
           plot = feature_plt,
           height = ft_ht, width = ft_wt, units = "in", dpi = 300, compression = "lzw")
    if(zip_figure == TRUE){
    
      system(paste0("gzip -f ", out_path, "/decon_refine_featurePlot.tiff"))  
    }
...
deconMapping.loc.plot <- function(data_path, 
  ...
  if (out_figure == TRUE){
    ...
    ggsave(filename =  paste0(out_path, "/decon_mapping_locPlot.tiff"), 
           plot = ct_plt,
           height = ct_ht, width = ct_wt, units = "in", dpi = 300, compression = "lzw")
    if(zip_figure == TRUE){
    
      system(paste0("gzip -f ", out_path, "/decon_mapping_locPlot.tiff"))  
    }
    print("Feature plot saved!")
  }
...
decon_plt.plot <- function(data_path1,                     ## String: output path for decon procedure 
  ...
  if (!file.exists(post_path)) {
    ...
    stop("Please run DECON module first!")
  } else {
    
    post_file <- read.table(post_path, header = F, sep = "\t")[, 1]
  }
  
  decon_method <- post_file[1]
  result_dir <- paste0(out_path, "/decon_result/", gsub("-", "_", decon_method), "/")
  if (!file.exists(result_dir)){
    
    system(paste0("mkdir ", result_dir))
  }
  ## plot
  decon_pie_plt <- pie.plot(data_path1 = data_path1,
                            data_path2 = data_path2,
                            pie_scale = 0.3,
                            out_path = result_dir,
                            out_figure = out_figures, 
                            zip_figure = zip_figures)
  prop_pattern_list <- propPattern.plot(data_path1 = data_path1,
                                        data_path2 = data_path2,
                                        out_path = result_dir,
                                        ct_sel = NULL,
                                        point_size = 0.1,
  ...
  source(paste0(method_path, "/dev/plt_utils.R"))
  ...
  save(prop_pattern_list, decon_pie_plt, decon_feature_plt, 
       file = paste0(result_dir, "/plot.RData"))
  
  if (decon_method == "CARD" & scMapping == TRUE){
