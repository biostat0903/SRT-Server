...
qc.check <- function(data_path = NULL,        ## String: path string of samples *****(required)
                     platform = NULL,         ## String: platform for st data *****(required)
                                              ## four platforms: {Visium}, {MERFISH}, {Seqfish}, {SlideseqV2}, {Generic_Cell}, {Generic_Spot}
...
LoadVisium_QC <- function(data_path,          ## path for st data
  ...
  ## read data
  spatial_img <- Read10X_Image(image.dir = paste0(data_path, "/spatial/"),
                               image.name = "tissue_hires_image.png")
  scale_factors <- jsonlite::fromJSON(txt = file.path(data_path, "/spatial/scalefactors_json.json"))
  spatial_obj <- Load10X_Spatial(data_path, image = spatial_img) 
  count_data <- as.matrix(spatial_obj@assays$Spatial@counts)
  meta_data <- spatial_obj@images$slice1@coordinates
  image_data <- spatial_img@image
  image_meta <- c("image_path" = paste0(data_path, "/spatial/tissue_hires_image.png"),
                  unlist(scale_factors)[c("fiducial_diameter_fullres",
...
qc.call <- function(data_path,                ## String: path for st data
  ...
  ## load spatial data
  check_file <- read.table(paste0(data_path, "/qc_check_file.txt"), sep = "\t")[, 1]
  ...
  if (sample_size != 1){
    ...
    spatial_data_all_inter <- plyr::llply(c(1: sample_size), function(a){
      ...
      spatial_data_a <- list(count = count_a, 
                             ...
                             image.data = spatial_data_all[[a]]$image.data,
                             image.meta = spatial_data_all[[a]]$image.meta)
