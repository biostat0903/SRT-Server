...
loc.plot <- function(data_path1,                   ## String: output path of ct procedure 
                     ...
                     remove_undef = T,             ## Boolean: whether to move "undef/unknown" cluster or not
  ...
  ct_loc_list <- lapply(clus_name, function(cl){
    ...
    if (remove_undef) {
      datt_f <- subset(datt,
                        !toupper(datt$cluster) %in% c("UNDEF", "UNKNOWN"))
    }
    
    datt_f$cluster <- factor(datt_f$cluster, 
                             levels = sort(unique(datt_f$cluster)))
    clus_num <- length(unique(datt_f$cluster))
    if (vis_type == "cell_type"){
      
      ct_loc_plt <- loc.visualize(datt = datt_f, 
                                  title_in = "", 
                                  vis_type = vis_type,
                                  pointsize = p_size,
                                  color_in = CT_COLS) 
      loc_name <- paste0(out_path, 
                         "/Location_ct_", cl, ".tiff")
    } else {
